        ...
        <div className="min-h-screen bg-[#050510] flex flex-col font-sans text-white">
            ...
            <div className="relative z-10 flex-1 w-full max-w-7xl mx-auto px-4 md:px-10 py-8 flex flex-col gap-6">
                ...
                <div className="text-center">
                    ...
                    <p className="text-sm md:text-lg text-red-100/40 font-mono tracking-widest uppercase">Global Revenue Sharing</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
                    ...
                    <div className="flex flex-col justify-center space-y-5">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }}
                            className="bg-gradient-to-br from-red-500 to-rose-600 p-0.5 rounded-3xl shadow-[0_0_50px_rgba(227,6,19,0.2)]">
                            <div className="bg-black/80 backdrop-blur-xl rounded-[22px] p-6 md:p-8 text-center border border-white/10">
                                <div className="text-gray-400 text-sm md:text-base uppercase tracking-wider mb-2 font-bold">Total Allocation</div>
                                <div className="text-6xl md:text-8xl font-black text-white mb-2">5%</div>
                                <div className="text-red-400 font-bold text-lg uppercase tracking-widest">of Global Revenue</div>
                            </div>
                        </motion.div>
                        ...
                        <ul className="space-y-4">
                            {[
                                ...
                                <motion.li key={i}
                                    ...
                                    className="flex items-start gap-4 p-4 bg-white/5 rounded-xl border border-white/5 hover:border-red-500/30 transition-colors">
                                    <span className="flex-shrink-0 w-9 h-9 rounded-full bg-red-500/20 flex items-center justify-center text-red-400 text-lg">★</span>
                                    <div>
                                        <strong className="block text-white text-sm md:text-base">{item.title}</strong>
                                        <span className="text-gray-400 text-sm font-bold uppercase tracking-tighter">{item.desc}</span>
                                    </div>
                                </motion.li>
                            ))}
                        </ul>

                        <div className="bg-red-900/20 rounded-2xl p-5 border border-red-500/20 text-center">
                            <span className="block text-red-300 text-xs uppercase tracking-widest mb-1 font-bold">Payout Frequency</span>
                            <span className="text-xl font-bold text-white uppercase tracking-tighter">Periodic Distribution</span>
                            <span className="block text-white/50 text-xs mt-1 uppercase tracking-tighter">(Automated via Smart Contract)</span>
                        </div>
                ...
                <div className="text-center pt-2 pb-4">
                    <Link href="/presentation"
                        className="inline-flex items-center gap-2 bg-gradient-to-r from-red-600 to-rose-800 text-white px-8 py-3 rounded-full font-black text-base shadow-lg shadow-red-500/30 hover:scale-105 transition-all border border-red-500/30">
            ...
            <div className="sticky bottom-0 z-50 bg-black/70 backdrop-blur-md border-t border-white/10 px-4 md:px-8 py-3 flex items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                    {['direct-referral', 'matrix-income', 'level-income', 'reward-pools'].map((s, i) => (
                        <div key={i} className={`w-2.5 h-2.5 rounded-full transition-all ${s === 'reward-pools' ? 'bg-red-500 scale-125 shadow-[0_0_8px_rgba(227,6,19,0.5)]' : 'bg-white/20'}`} />
                    ))}
                </div>
                <Link href="/presentation"
